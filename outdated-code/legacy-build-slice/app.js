...
const SLICE_CYCLES = window.GAME_CONFIG.slice.cycles;
const SLICE_BUILDS = window.GAME_CONFIG.slice.builds;
const SLICE_OPPORTUNITY_BUILDS = window.GAME_CONFIG.slice.opportunityBuilds;
...
const elements = {
  ...
  opportunityScreen: document.querySelector("#opportunity-screen"),
  ...
  opportunityKicker: document.querySelector("#opportunity-kicker"),
  opportunityTitle: document.querySelector("#opportunity-title"),
  opportunityCrisis: document.querySelector("#opportunity-crisis"),
  opportunityBrief: document.querySelector("#opportunity-brief"),
  opportunityGrid: document.querySelector("#opportunity-grid"),
  opportunityCount: document.querySelector("#opportunity-count"),
  opportunityConfirm: document.querySelector("#opportunity-confirm"),
...
function newState() {
  return {
    ...
    sliceCycle: -1,
    sliceCyclePlayed: 0,
    sliceCycleStartStats: null,
    slicePendingSelection: [],
    sliceOmitted: [],
    sliceSynergies: [],
    sliceCycleSynergies: [],
    sliceCrisisChoices: 0,
    tutorialDismissed: [],
...
function getCard(cardId) {
  ...
  if (state?.sliceMode && ["C01", "C02", "C03"].includes(cardId)) {
    applySliceCrisisVariant(cardId, card);
  }

  if (cardId === "S02" && hasTag("selfStart")) {
    card.body = `会议室已经变成研发室。${card.body}`;
    card.choices[1].effects.tech += 4;
    card.choices[1].result += " 昨天刚腾出的研发室，让这个计划比预期更快启动。";
  }

  if (cardId === "S02" && hasTag("serviceBusiness")) {
    card.body = `出租机器带来了第一批客户。${card.body}`;
    card.choices[0].effects.cash += 3;
    card.choices[0].effects.market += 1;
    card.choices[0].result += " 第一批租用机器的客户已经替你介绍了这笔生意。";
  }
...
function beginAnnualMarket(reportYear) {
  if (reportYear === annualConfig.startYear && state.marketCondition?.year !== reportYear) {
    state.marketCondition = { year: reportYear, multiplier: 1, label: "平稳行情", marketChange: 0 };
    return;
  }
...
function showOnly(section) {
  [elements.startScreen, elements.gameScreen, elements.opportunityScreen, elements.newspaperScreen, elements.endingScreen]
...
function crisisProgressFor(year) {
  const crisisYear = year <= 1982 ? 1982 : 2001;
  const distance = crisisYear - year;
  if (distance <= 0) return year <= 1982 ? "科技寒冬：今年" : "泡沫危机：今年";
  return `${year < 1982 ? "科技寒冬" : "泡沫危机"}：${distance} 年后`;
}

function sliceMarker(cardId, prefix) {
  return cardId?.startsWith(`${prefix}:`) ? Number(cardId.split(":")[1]) : null;
}

function sliceBuildScore(buildId) {
  return SLICE_BUILDS[buildId].tags.filter((tag) => hasTag(tag)).length;
}

function opportunityAbilityNames(card) {
  return [...new Set(card.choices.flatMap((item) => item.add || []))]
    .map((tag) => tagNames[tag])
    .filter(Boolean);
}

function applySlicePriorityBonus(effects) {
  const isFirstCycleAction = state.sliceMode
    && state.sliceCycle >= 0
    && state.sliceCycle < SLICE_CYCLES.length
    && state.sliceCyclePlayed === 0;
  if (!isFirstCycleAction) return null;

  const primaryStat = VISIBLE_STATS
    .filter((key) => (effects[key] || 0) > 0)
    .sort((left, right) => effects[right] - effects[left])[0];
  if (!primaryStat) return null;
  effects[primaryStat] += 3;
  return primaryStat;
}

function dominantSliceBuild() {
  return Object.keys(SLICE_BUILDS)
    .sort((left, right) => sliceBuildScore(right) - sliceBuildScore(left))[0];
}

function activateSliceSynergies() {
  if (!state.sliceMode) return [];
  const activated = Object.entries(SLICE_BUILDS)
    .filter(([buildId]) => sliceBuildScore(buildId) >= 3 && !state.sliceSynergies.includes(buildId))
    .map(([buildId, build]) => {
      state.sliceSynergies.push(buildId);
      const effects = applyEffects(build.bonus);
      const synergy = { id: buildId, name: build.name, body: build.summary, effects };
      state.sliceCycleSynergies.push(synergy);
      return synergy;
    });
  return activated;
}

function applySliceCrisisVariant(cardId, card) {
  const buildId = dominantSliceBuild();
  const build = SLICE_BUILDS[buildId];
  const variants = {
    C01: {
      tech: choice("发布差异化产品", { cash: 5, tech: -4, market: 7 }, "新产品避开价格战", "同行在清理库存时，你的新产品仍然有人排队。技术第一次替财务部门说服了银行。", ["researchEmber"]),
      customer: choice("执行长期合同", { cash: 8, talent: -6, market: 4 }, "老客户继续履行合同", "销售网络保住了订单。交付团队表示，他们也很希望有人顺便保住睡眠。", ["channelSurvivor"]),
      manufacturing: choice("控制库存出货", { cash: 4, talent: -2, market: 7 }, "供应体系维持交付", "同行缺货时，你的仓库仍能发货。仓库主管第一次被邀请参加庆功会。", ["inventoryControlled"])
    },
    C02: {
      tech: choice("授权非核心技术", { cash: 15, tech: -5, market: 2 }, "技术授权换来现金", "公司卖掉一部分过去，没有卖掉下一代产品。", ["crisisLoan"]),
      customer: choice("向客户收取预付款", { cash: 14, talent: -5, market: 3 }, "客户提前支付救命订单", "长期关系终于变成银行愿意承认的资产。", ["crisisLoan"]),
      manufacturing: choice("抵押生产能力", { cash: 16, market: -3 }, "生产线换来紧急融资", "银行不理解芯片，但能够理解厂房和土地。", ["crisisLoan"])
    },
    C03: {
      tech: choice("保住研发引擎", { cash: -7, tech: 10, talent: 6, market: -6 }, "公司保住技术先锋体系", "产品线缩小了，能够制造下一代产品的人仍然坐在办公室里。", ["researchEmber"]),
      customer: choice("保住客户网络", { cash: 6, tech: -7, talent: -4, market: 10 }, "公司保住客户机器体系", "研发计划被削减，但客户仍然知道下一份订单该打给谁。", ["channelSurvivor"]),
      manufacturing: choice("保住供应体系", { cash: -5, tech: 3, talent: 4, market: 8 }, "公司保住韧性制造体系", "工厂降低了产量，却没有关灯。供应商也没有挂断电话。", ["coreProtected"])
    }
  };
  const selectedVariant = variants[cardId]?.[buildId];
  if (!selectedVariant || sliceBuildScore(buildId) < 2) return;
  selectedVariant.sliceBuild = buildId;
  card.choices[0] = selectedVariant;
  card.body += ` 你建立的“${build.name}”能力提供了一条额外生路。`;
}

function renderOpportunitySelection(cycleIndex) {
  const cycle = SLICE_CYCLES[cycleIndex];
  state.sliceCycle = cycleIndex;
  state.view = "opportunities";
  state.pendingResult = null;
  state.slicePendingSelection = state.slicePendingSelection || [];
  showOnly(elements.opportunityScreen);
  renderStats();
  elements.opportunityKicker.textContent = `${cycle.year} · 经营周期 ${cycleIndex + 1}/${SLICE_CYCLES.length}`;
  elements.opportunityTitle.textContent = cycle.title;
  elements.opportunityCrisis.textContent = `距离科技寒冬：${SLICE_CYCLES.length - cycleIndex} 个周期`;
  elements.opportunityBrief.textContent = cycle.brief;

  const refreshSelection = () => {
    const selected = state.slicePendingSelection;
    [...elements.opportunityGrid.children].forEach((button) => {
      const order = selected.indexOf(button.dataset.cardId);
      button.classList.toggle("selected", order >= 0);
      button.querySelector(".opportunity-order").textContent = order >= 0 ? `${order + 1}` : "";
    });
    elements.opportunityCount.textContent = selected.length === 2
      ? "1 号重点投入：主要收益 +3 · 2 号常规推进"
      : `选择 2 项，并决定处理顺序 · 已选 ${selected.length}/2`;
    elements.opportunityConfirm.disabled = selected.length !== 2;
    saveState();
  };

  elements.opportunityGrid.innerHTML = "";
  cycle.opportunities.forEach((opportunity) => {
    const card = getCard(opportunity.id);
    const buildHint = (SLICE_OPPORTUNITY_BUILDS[opportunity.id] || [])
      .map((buildId) => {
        const score = sliceBuildScore(buildId);
        return `${SLICE_BUILDS[buildId].name}（${score >= 3 ? "已成型" : `当前 ${score}/3`}）`;
      })
      .join(" · ");
    const abilities = opportunityAbilityNames(card);
    const button = document.createElement("button");
    button.type = "button";
    button.className = "opportunity-card";
    button.dataset.cardId = opportunity.id;
    button.innerHTML = `
      <span class="opportunity-order" aria-hidden="true"></span>
      <span class="opportunity-deadline">期限 · ${opportunity.deadline}</span>
      <strong>${card.title}</strong>
      <span class="opportunity-build">${buildHint ? `对应路线 · ${buildHint}` : "通用经营机会"}</span>
      <span class="opportunity-ability">可能能力 · ${abilities.length ? abilities.join(" / ") : "无永久能力"}</span>
      <span class="opportunity-payoff">收益 · ${opportunity.payoff}</span>
      <span class="opportunity-risk">风险 · ${opportunity.risk}</span>`;
    button.addEventListener("click", () => {
      const selectedIndex = state.slicePendingSelection.indexOf(opportunity.id);
      if (selectedIndex >= 0) state.slicePendingSelection.splice(selectedIndex, 1);
      else if (state.slicePendingSelection.length < 2) state.slicePendingSelection.push(opportunity.id);
      refreshSelection();
    });
    elements.opportunityGrid.appendChild(button);
  });

  elements.opportunityConfirm.onclick = () => {
    if (state.slicePendingSelection.length !== 2) return;
    const selected = [...state.slicePendingSelection];
    state.sliceOmitted = cycle.opportunities
      .filter((opportunity) => !selected.includes(opportunity.id))
      .map((opportunity) => getCard(opportunity.id).title);
    state.sliceCycleStartStats = clone(state.stats);
    state.sliceCycleSynergies = [];
    state.sliceCyclePlayed = 0;
    state.slicePendingSelection = [];
    state.sequence.splice(state.index, 1, ...selected, `SLICE_SETTLE:${cycleIndex}`);
    state.view = "card";
    saveState();
    renderCard();
  };
...
function renderSliceSettlement(cycleIndex) {
  const cycle = SLICE_CYCLES[cycleIndex];
  const changes = Object.fromEntries(VISIBLE_STATS.map((key) => [key, state.stats[key] - state.sliceCycleStartStats[key]]));
  const synergyText = state.sliceCycleSynergies.length
    ? `本周期启动：${state.sliceCycleSynergies.map((synergy) => synergy.name).join("、")}。`
    : "本周期没有形成新的能力协同。";
  state.pendingResult = {
    year: cycle.year,
    headline: `${cycle.title} · 周期报告`,
    body: `${synergyText} 未处理的机会：${state.sliceOmitted.join("、")}。机会已经离开，董事会将其重新命名为“战略性放弃”。`,
    effects: changes,
    records: state.sliceCycleSynergies.map((synergy) => `协同：${synergy.name}`),
    source: `SLICE_SETTLE:${cycleIndex}`
  };
  state.view = "result";
  saveState();
  renderResult();
...
function renderCard() {
  ...
  const selectionCycle = sliceMarker(cardId, "SLICE_SELECT");
  if (selectionCycle !== null) {
    renderOpportunitySelection(selectionCycle);
    return;
  }
  const settlementCycle = sliceMarker(cardId, "SLICE_SETTLE");
  if (settlementCycle !== null) {
    renderSliceSettlement(settlementCycle);
    return;
  }
  if (cardId === "SLICE_END") {
    showSliceEnding();
  ...
  const crisisIndex = ["C01", "C02", "C03"].indexOf(cardId);
  const startsNewYear = state.sliceMode
    ? cardId === "S01" || (crisisIndex === 0) || (state.sliceCycle < SLICE_CYCLES.length && state.sliceCyclePlayed === 0)
    : state.index % 4 === 0;
  ...
  if (state.sliceMode && state.sliceCycle >= 0 && state.sliceCycle < SLICE_CYCLES.length) {
    card.year = SLICE_CYCLES[state.sliceCycle].year;
  }

  if (startsNewYear && !state.sliceMode) {
  ...
  elements.arrivalNote.textContent = state.sliceMode ? "关键经营周期" : "年度计划开始";
  ...
  elements.progress.textContent = state.sliceMode
    ? (crisisIndex >= 0
      ? `科技寒冬 · 危机 ${crisisIndex + 1}/3`
      : state.sliceCycle < 0
        ? `创业 ${state.index + 1}/2 · 科技寒冬：4 个周期后`
        : `本期 ${state.sliceCyclePlayed + 1}/2 · ${state.sliceCyclePlayed === 0 ? "重点项目（主要收益 +3）" : "常规项目"}`)
    : `${crisisProgressFor(card.year)} · 本年 ${state.index % 4 + 1}/4`;
  ...
  options.forEach((item, choiceIndex) => {
    ...
    let displayedEffects = showDeveloperValues ? formatEffects(previewEffects) : formatDirections(previewEffects);
...
function resolveChoice(cardId, card, choiceIndex) {
  ...
  const resolvedEffects = resolvedChoiceEffects(selected);
  const priorityStat = applySlicePriorityBonus(resolvedEffects);
  const effects = applyEffects(resolvedEffects);
  removeTags(selected.remove);
  addTags(selected.add);

  if (selected.sliceBuild) state.sliceCrisisChoices += 1;
  const synergies = activateSliceSynergies();
  synergies.forEach((synergy) => {
    Object.entries(synergy.effects).forEach(([key, value]) => {
      effects[key] = (effects[key] || 0) + value;
    });
  });
  const newRecords = [
    ...selected.add.map((tag) => tagNames[tag]).filter(Boolean),
    ...synergies.map((synergy) => `协同启动：${synergy.name}`)
  ];
  state.history.push({
    year: card.year,
    card: card.title,
  ...
  const annualReportPending = state.sliceMode ? false : isFinalAnnualDecision(cardId, card.year);
  ...
  state.pendingResult = {
    ...
    body: [
      selected.result,
      priorityStat ? `重点项目获得额外资源，${STAT_NAMES[priorityStat]}收益提高。` : "",
      ...synergies.map((synergy) => `${synergy.name}启动：${synergy.body}`)
    ].filter(Boolean).join(" "),
    effects,
    records: newRecords,
    source: cardId,
...
function renderResult() {
  ...
  if (result.year < 1980) {
    ...
    elements.nextHint.textContent = isAnnualReport ? `继续：进入 ${result.year + 1} 年` : "继续：下一页";
    ...
    elements.nextHint.textContent = isAnnualReport ? `继续：进入 ${result.year + 1} 年` : "继续：下一条";
  } else {
    ...
    elements.nextHint.textContent = isAnnualReport ? `继续：进入 ${result.year + 1} 年` : "继续：下一条消息";
  }
  elements.resultMedium.classList.toggle("synergy-result", Boolean(result.synergy));
  ...
  if (["C03", "C06"].includes(result.source)) elements.nextHint.textContent = "继续：查看危机结果";
  if (result.annualReportPending) elements.nextHint.textContent = `继续：查看 ${result.year} 年年报`;
  setupResultSwipe();
}
...
function nextSliceStep(source) {
  const settledCycle = sliceMarker(source, "SLICE_SETTLE");
  if (settledCycle !== null) {
    const nextCycle = settledCycle + 1;
    state.index += 1;
    state.sliceCycle = nextCycle;
    state.sliceCyclePlayed = 0;
    state.pendingResult = null;
    state.view = "card";
    if (nextCycle < SLICE_CYCLES.length) {
      state.sequence.splice(state.index, 0, `SLICE_SELECT:${nextCycle}`);
    } else {
      state.sequence.splice(state.index, 0, "C01", "C02", "C03", "SLICE_END");
    }
    saveState();
    renderCard();
    return;
  ...
  state.index += 1;
  if (state.sliceCycle >= 0 && state.sliceCycle < SLICE_CYCLES.length) {
    state.sliceCyclePlayed += 1;
  }
  state.pendingResult = null;
  state.view = "card";
  saveState();
...
function showFailure(forcedType) {
  ...
  const sliceBuildId = state.sliceMode ? dominantSliceBuild() : null;
  const sliceContext = sliceBuildId ? `
    <section class="career-summary">
      <h3>公司留下的路线</h3>
      <p>最接近的 Build：${SLICE_BUILDS[sliceBuildId].name}（${sliceBuildScore(sliceBuildId)} 项能力）</p>
      <p>已启动协同：${state.sliceSynergies.length ? state.sliceSynergies.map((id) => SLICE_BUILDS[id].name).join("、") : "无"}</p>
      <p>下次应优先补足导致归零的能力，并调整机会处理顺序。</p>
    </section>` : "";
...
function showSliceEnding() {
  const buildId = dominantSliceBuild();
  const build = SLICE_BUILDS[buildId];
  const buildComplete = state.sliceSynergies.includes(buildId);
  const crisisReady = state.sliceCrisisChoices >= 2;
  const successful = buildComplete && crisisReady;
  const recentChoices = state.history.slice(-3)
    .map((entry) => `<li><strong>${entry.year} · ${entry.card}</strong>：${entry.decision}</li>`)
    .join("");

  showOnly(elements.endingScreen);
  renderStats();
  elements.endingScreen.classList.toggle("failure-ending", !successful);
  elements.endingScreen.innerHTML = `
    <p class="kicker">1982 · ${successful ? `${build.name}结局` : "幸存公司"}</p>
    <h2>${successful ? "这套体系跑起来了。" : "公司活下来了，体系还没有。"}</h2>
    <p class="ending-copy">${successful
      ? `${build.summary} 科技寒冬检验了这套组合，它没有依靠一道标准答案活下来。`
      : `公司勉强穿过寒冬，但没有形成足以主导危机的完整能力组合。当前最接近的方向是“${build.name}”。`}</p>
    <section class="career-summary">
      <h3>最后三项决定</h3>
      <ul>${recentChoices}</ul>
      <p>已启动协同：${state.sliceSynergies.length ? state.sliceSynergies.map((id) => SLICE_BUILDS[id].name).join("、") : "无"}</p>
      <p>危机中使用 Build 解法：${state.sliceCrisisChoices} / 3</p>
    </section>
    <div class="ending-actions">
      <button id="restart-button" class="primary-button" type="button">更换创业方向</button>
    </div>`;
  state.view = "slice-ending";
  state.endingType = buildId;
  saveState();
  document.querySelector("#restart-button").addEventListener("click", startNewGame);
}
...
function continueGame() {
  ...
  else if (state.view === "slice-ending") showSliceEnding();
  else renderCard();
}
...
elements.nextHint.addEventListener("click", (event) => {
